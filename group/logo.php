...
//Alter these options
	...
	if ( !defined('IN_ZUS') )
	{
		exit('<h1>503:Service Unavailable @group:logo</h1>');
	}
	...
	if (empty($gid))
	{
		Unicorn::show($home, '@group>logo.php: Undefine $id');
	}
	else
	{
		$logo_obj = GroupDAO::get_group_logo($auth['uid'], $gid);
	...
	$file = 'upload/'.$pid;
	$temp_file_dir = 'upload/temp/'.$pid.'/group'.'/'.$gid;
	...
	$large_image_name = "default_group_large.jpg"; 		// New name of the large image
	...
	{
		$large_image_location = $home."theme/images/default/".$large_image_name;
	}
	...
	if (isset($_POST["upload"]))
	{
		...
		if (strlen($error)==0)
		{
			...
			header('Location: logo.php?gid='.$gid);
	...
	if (isset($_POST["upload_thumbnail"]) && strlen($large_photo_exists)>0)
	{
		...
		GroupDAO::set_group_logo_gid($gid, $save_path);
