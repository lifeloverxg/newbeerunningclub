<?php
	...
	if ( !empty($search_result) )
	{
		...
		$args['more'] = "<a href='javascript: search_function_relocation(".$args['pid'].")'>查看更多</a>";
	}
	else
	{
		switch ( $args['type'] ) 
		{
			case '全部':
				...
				{
					foreach($search_result_all as $key => $value)
					{
						if ( !empty($value) )
						{
							$args['list'] .= "<div class='search-result-category-title'>"."找到的-".$key.":"."</div>";
							foreach($value as $result)
							{
								$args['list'] .= "<li class='li-search-result-event-group'>
													<a href='".$home.$result['url']."'>
														<div class='event-group-search-left-area'>
															<img class='logo-small' src='".$home.$result['image']."' alt='".$result['alt']."' title='".$result['title']."'>
														</div>
