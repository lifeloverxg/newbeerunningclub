...
class TCPDFBarcode {
	...
	public function getBarcodeHTML_Sean($w=2, $h=30, $color='black') {
		$html = '<div style="font-size:0; display:block; float:left; position:relative;height:'.($this->barcode_array['maxw'] * $w).'px;width:'.($h).'px;">'."\n";
		// print bars
		$x = 0;
		foreach ($this->barcode_array['bcode'] as $k => $v) {
			$bh = round(($v['w'] * $w), 3);
			$bw = round(($v['h'] * $h / $this->barcode_array['maxh']), 3);
			if ($v['t']) {
				$y = round(($v['p'] * $h / $this->barcode_array['maxh']), 3);
				// draw a vertical bar
				$html .= '<div style="background-color:'.$color.';width:'.$bw.'px;height:'.$bh.'px;display:block; float:left; margin-bottom: 2px; position:inherit;top:'.$y.'px;"></div>'."\n";
			}
			$x += $bw;
		}
		$html .= '</div>'."\n";
